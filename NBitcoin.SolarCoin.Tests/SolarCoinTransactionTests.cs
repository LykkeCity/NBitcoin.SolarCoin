...
namespace NBitcoin.SolarCoin.Tests
{
    public class SolarCoinTransactionTests
    {
        ...
        //}
        [Fact]
        public void ShouldBuildAndSignV4()
        {
            EnvInitializer.Init();
            ...
            var coins = new Coin[]
            {
                new Coin(new OutPoint(uint256.Parse(sourceTxHash), n),
                    new TxOut(new Money(sourceSatoshi, MoneyUnit.Satoshi),
                        sourceTransaction.Outputs[n].ScriptPubKey))
            };

            //if (fromAddressPubkey != null)
            //{
            //    var reedem = fromAddressPubkey.WitHash.ScriptPubKey;
            //    coins = coins.Select(p => p.ToScriptCoin(reedem)).ToArray();
            //}

            var addressFrom = keyFrom.PubKey.GetAddress(_solarNetwork).ToString();
            var addressDest = keyTo.PubKey.GetAddress(_solarNetwork).ToString();

            BitcoinAddress from = BitcoinAddress.Create(addressFrom,
                _solarNetwork);
            ...
            var rawTransactionSigned = txSigned.ToHex();
